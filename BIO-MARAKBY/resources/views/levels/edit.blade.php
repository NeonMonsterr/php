        ...
        <div class="flex-1 p-4 md:p-6 md:mr-64">
            <div class="max-w-3xl mx-auto p-6 md:p-8 glass-box">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-xl md:text-2xl font-bold text-gray-900">✏️ تعديل المستوى</h1>
                ...
                <form action="{{ route('levels.update', $level) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                                  class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none">{{ old('description', $level->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">⚠️ {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end">
                        <button type="submit"
                                class="bg-gradient-to-r from-yellow-500 to-amber-500 text-white px-6 py-2 rounded-lg font-semibold hover:from-yellow-600 hover:to-amber-600 transition">
                            💾 حفظ التعديلات
                        </button>
                    </div>
                </form>
