    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-4 sm:p-6">
            <div class="max-w-3xl mx-auto card">
                ...
                <p class="text-gray-700 mb-2"><strong>المرحلة:</strong> {{ $course->stage->name ?? 'غير محدد' }}</p>
                <p class="text-gray-700 mb-2"><strong>المستوى:</strong> {{ $course->level->name ?? 'غير محدد' }}</p>
                            ...
                            @foreach ($lectures as $lecture)
                                <li class="bg-gray-50 p-4 rounded-lg shadow-sm flex justify-between items-center">
                                    <div>
                                        ...
                                        @if (auth()->user()->role === 'teacher')
                                            <p class="text-gray-500 text-sm">الحالة: {{ $lecture->is_published ? 'منشور' : 'غير منشور' }}</p>
                                        @endif
                                    </div>
